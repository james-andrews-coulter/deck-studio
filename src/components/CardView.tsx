...
export function CardView({ card, mapping, className }: Props) {
  ...
  return (
    <article className={`rounded-xl border bg-white p-3 shadow-sm dark:bg-slate-900 ${className ?? ''}`}>
      {r.image && <img src={r.image} alt="" className="mb-3 max-h-32 w-full rounded-md object-cover" />}
